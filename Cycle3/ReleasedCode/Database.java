...
public class Database {
    ...
    public void menu() {
        System.out.println("What would you like to do? Search or add member or find relationships. Enter QUIT if you'd like to exit");
        String answer = scanner.nextLine().toUpperCase();
        if (answer.contains("QUIT")) {
            System.out.println("Thanks for checking out our Genealogy project!");
            System.exit(0);
        }
        else if (answer.contains("SEARCH")) {
            searchMember();
        }
    ...
    public void searchMember() {
        System.out.println("Enter keycode to proceed: \n Search by a person's first name: 1  \n Search by a person's last name: 2 \n Search by a person's ID number: 3");
        String searchName = scanner.nextLine();
        if (searchName.equals("1")) { //search by first name
            int count = 0;
            System.out.println("Enter person's first name");
            searchName = scanner.nextLine();
            System.out.println("Here are all persons with this first name in the current tree:");
            for (Person i : persons.values()) {
                String firstname = i.getFirstName();
                if (firstname.equals(searchName)) {
                    count++;
                    System.out.println(i.getFirstName() + " " + i.getLastName() + " " + i.getNameSuffix());
                }
            }
            if (count == 0){
                System.out.println(searchName + " was not found in the current tree; please see below on how to proceed");
                menu();
            }


            System.out.println("To find more information about a particular person, copy and paste their full name into the next line");
            searchName = scanner.nextLine();
            for (Person i : persons.values()) {
                // TODO possibly make this "more information" part a separate function to not repeat common code!
        ...
        if (searchName.equals("2")) { //search by last name
            int count = 0;
        ...
        if (searchName.equals("3")) { //search by person id
            int count = 0;
            System.out.println("Enter person's id number");
            searchName = scanner.nextLine().toUpperCase();
            for (String i : persons.keySet()) {
                if (i.equals(searchName)) {
                    count++;
                    System.out.println(persons.get(i).getFirstName() + " " + persons.get(i).getLastName() + " " + persons.get(i).getNameSuffix() + " was found. Birth and death information listed below:");
                    if (persons.get(i).getBirthday().equals(" ")) {
                        System.out.println(persons.get(i).getFirstName() + "'s birthday or is not listed");
                    } else {
                        System.out.println("Born: " + persons.get(i).getBirthday() + " in " + persons.get(i).getBirthCity());
                    }
                    if (persons.get(i).getDeathDate().equals(" ")) {
                        System.out.println(persons.get(i).getFirstName() + " 's death day is not listed");
                    } else {
                        System.out.println("Died: " + persons.get(i).getDeathDate() + " in " + persons.get(i).getDeathCity());
                    }
                }
...
// test cases: p1 (The siblings of Dick Johnson Jr are listed below: Jane Sarah Johnson Sally Abigale Johnson BVM)
    ...
    public void addMember(){
        ...
        System.out.println("The order is as follows: person ID Num, last name, first name, suffix, birthday, birth city, death day, death city, parent relation ID Num");
